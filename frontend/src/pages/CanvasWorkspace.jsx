...
function CanvasWorkspace() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.ctrlKey && e.key === 'y') { e.preventDefault(); handleAddYoutube() }
      if (e.ctrlKey && e.key === 'n') { e.preventDefault(); handleAddNotes() }
      if (e.ctrlKey && e.key === 'i') { e.preventDefault(); handleAddAi() }
      if (e.ctrlKey && e.key === 'p') { e.preventDefault(); handleAddPdf() }
      if (e.ctrlKey && e.key === 'q') { e.preventDefault(); handleAddQuiz() }
    }

    window.addEventListener('keydown', handleKeyDown)
    return () => window.removeEventListener('keydown', handleKeyDown)
  }, [showHighlightAnswer, id])
  ...
  const handleAddQuiz = () => addPanel(id, 'QUIZ', {})
    ...
    <div style={styles.container} onMouseUp={handleMouseUp}>
      <div style={styles.toolbar}>
        ...
        <div style={styles.toolbarActions}>
          <button style={styles.addBtn} onClick={handleAddYoutube} title="Ctrl+Y">+ YouTube</button>
          <button style={styles.addBtn} onClick={handleAddNotes} title="Ctrl+N">+ Notes</button>
          <button style={styles.addBtn} onClick={handleAddAi} title="Ctrl+I">+ AI</button>
          <button style={styles.addBtn} onClick={handleAddPdf} title="Ctrl+P">+ PDF</button>
          <button style={styles.addBtn} onClick={handleAddQuiz} title="Ctrl+Q">🧠 Quiz</button>
        </div>

        {/* Right — shortcuts hint + save status */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
          ...
          <div style={{ fontSize: '10px', color: '#B0A890', display: 'flex', gap: '8px' }}>
            <span>Ctrl+N Notes</span>
            <span>Ctrl+Y YouTube</span>
            <span>Ctrl+I AI</span>
            <span>Ctrl+P PDF</span>
            <span>Ctrl+Q Quiz</span>
            <span>Esc Close</span>
          </div>
        </div>
      </div>
          ...
          {panels.length === 0 && (
            <div style={styles.emptyState}>
              ...
              <p style={{ color: '#B0A890', fontSize: '13px', marginTop: '6px' }}>
                Add a YouTube, Notes, AI, PDF or Quiz panel to get started
              </p>
            </div>
          )}
        </div>
